...
横屏手机优先的双人异步在线大富翁原型。Host 是权威端，负责骰子、位置、金币、活力、房产、落点过路费、奖励和转盘结果；Client 只发送操作请求。两名玩家不再轮流等待，可同时行动。

## 地图与经济

- 经济系统仅使用金币；所有经济数值和 30 格类型配置集中在 `scripts/game_rules.gd`。
- 两名玩家初始金币均为 `1000`，初始活力均为 `20`。每次掷骰消耗 1 点活力，只有自己的状态为 `IDLE` 时才能再次掷骰。
- 建造费用 L1～L5：`50 / 100 / 200 / 400 / 800`。
- 过路费 L1～L5：`25 / 50 / 100 / 200 / 400`。仅最终落在敌产时收取，Host 转账并让双方显示提示；付款玩家确认后再处理抢占。
- `5 / 12 / 19 / 26` 为奖励格，最终停下获得 100 金币。
- `9 / 22` 为转盘格。Host 先抽取统一结果，双方显示六扇区转盘并播放相同目标的减速旋转动画。
...
验收覆盖：玩家独立活力与状态、同时移动、重复请求拦截、最终落点事件、同格房产冲突串行化、奖励、Host 转盘结果以及 Host/Client 最终快照一致性。
