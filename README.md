...
- 经济系统仅使用金币；所有经济数值和 50 格类型配置集中在 `scripts/game_rules.gd`。
...
- 地图包含 25 个房产格（20 HOUSE / 5 HOTEL）、15 个答题格、5 个奇遇格、2 个转盘格和 2 个商店格，不再放置奖励格。
...
- 每名玩家的落点事件独立处理，确认完成后自己的状态恢复为 `IDLE`；其他玩家不受阻塞。
- 财富排行榜保留存档中所有曾加入玩家并标注在线/离线；Host 可用调试修改器直接调整金币、活力和卡牌。
...
└─ tests/
   ...
   ├─ map_debug_test.gd # 50 格、离线榜单、修改器与版本失效验收
   └─ network_peer_test.gd # Host + 3 Client 联机验收
```
...
- 默认路线为 50 个显式坐标组成的凹凸闭环，多次转弯且 49→0 连续；范围约 4590×2460px，大于 1920×1080 设计视口。
