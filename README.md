...
横屏手机优先的双人联机大富翁原型。Host 是权威端，负责回合、骰子、位置、金币、房产、逐格过路费、奖励和转盘结果；Client 只发送操作请求。
...
- 过路费 L1～L5：`25 / 50 / 100 / 200 / 400`。棋子到达敌产格后暂停，Host 转账并让双方显示提示；付款玩家确认后才继续移动。
- `5 / 12 / 19 / 26` 为奖励格，最终停下获得 100 金币。
- `9 / 22` 为转盘格。Host 先抽取统一结果，双方显示六扇区转盘并播放相同目标的减速旋转动画。
- 最终事件确认前不会切换玩家；路过奖励格或转盘格不会触发事件。
...
验收覆盖：集中经济表、格子类型、逐格过路费、连续收费、买地、升级、抢占、奖励、Host 转盘结果、事件顺序以及 Host/Client 最终快照一致性。
